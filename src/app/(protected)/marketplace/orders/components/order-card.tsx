...
import { Calendar, ChevronRight, Check, RotateCcw } from "lucide-react";
...
export default function OrderCard({ order }: Props) {
  ...
  const primaryImage =
    firstItem?.product.images.find((image) => image.isPrimary)?.url ??
  ...
  return (
    <article className="rounded-2xl border border-[#ECE9F6] bg-white p-5 transition-colors hover:border-violet-200 hover:shadow-[0_12px_32px_-16px_rgba(124,58,237,0.2)] sm:p-6">
      ...
      {firstItem && (
        <div className="flex gap-4 py-5">
          <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-[#F8F8FC]">
            {primaryImage && (
              <Image src={primaryImage} alt={firstItem.product.title} fill className="object-cover" />
            )}
          </div>
      ...
      <div className="grid gap-4 border-t border-[#F2F1F8] py-4 sm:grid-cols-2">
